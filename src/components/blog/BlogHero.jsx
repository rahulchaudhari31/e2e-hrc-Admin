...
import {
  ...
  updateBlogHero,
  deleteBlogHero,
} from '../../services/blog/blogHeroService';
...
export default function BlogHero() {
  ...
  const loadRecord = async () => {
    setIsLoading(true);
    try {
      const response = await getBlogHeroes();
      ...
      toast.error(error.message || 'Failed to load Blog Hero');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let response;
      if (editingId) {
        response = await updateBlogHero(editingId, payload);
        if (response?.success) {
          toast.success('Blog Hero updated successfully!');
        }
      } else {
        response = await createBlogHero(payload);
        if (response?.success) {
          toast.success('Blog Hero created successfully!');
      ...
      const msg = error?.message || (editingId ? 'Failed to update Blog Hero.' : 'Failed to create Blog Hero.');
      toast.error(msg);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await deleteBlogHero(editingId);
      if (response?.success) {
        toast.success('Blog Hero deleted successfully!');
        setConfirmDelete(false);
        resetForm();
      }
    } catch (error) {
      toast.error(error?.message || 'Failed to delete Blog Hero.');
    } finally {
      setDeletingId(null);
    }
  };
