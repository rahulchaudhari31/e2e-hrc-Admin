...
import { Download, Eye, Inbox, Loader2, Trash2, UserRound, X } from "lucide-react";
...
} from "../../services/home/employeeApi";
...
const notifyNewEnquiries = (items) => {
  ...
  localStorage.setItem("employeeEnquiriesNewCount", String(count));
  window.dispatchEvent(new CustomEvent("employee-enquiries-count-changed", { detail: { count } }));
};

export default function EmployeeEnquiries() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to load employee enquiries");
    } finally {
      setLoading(false);
    }
  };
  ...
  const viewDetails = async (item) => {
    try {
      setSelected(await getEmployeeById(item._id));
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success("Employee enquiry deleted successfully");
    } catch (error) {
      toast.error(error.message || "Failed to delete employee enquiry");
    } finally {
      setDeletingId(null);
    }
  };
    ...
    <section className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center gap-3 p-12">
          ...
          <p className="text-sm text-gray-500">Loading employee enquiries...</p>
        </div>
      ) : items.length === 0 ? (
        <div className="flex flex-col items-center p-12 text-center">
          <Inbox size={28} className="text-orange-300" />
          <p className="mt-3 font-medium text-gray-600">
            No employee enquiries found
          </p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {items.map((item) => (
                <tr key={item._id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4 hidden lg:table-cell text-sm text-gray-700">
                    {item.location}
                  </td>
                  <td className="p-4">
                    <select
