...
export default function EmployeeWhyChooseSectionManager() {
  ...
  const fetchSection = async () => {
    try {
      setIsLoading(true);
      const res = await getEmployeeWhyChooseSections();
  ...
  const saveSection = async () => {
    ...
    try {
      if (sectionId) {
        await updateEmployeeWhyChooseSection(sectionId, formData);
        toast.success('Section updated successfully!');
      } else {
        await createEmployeeWhyChooseSection(formData);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8 p-6">
      {/* Header / Toggle */}
      <div
        className="flex items-center justify-between pb-6 border-b border-gray-200 cursor-pointer"
        onClick={() => setIsSectionExpanded(!isSectionExpanded)}
      >
        <div>
          <h3 className="text-lg font-bold text-gray-800">Employee Why Choose Section</h3>
