...
} from '../../services/workforceSolution/testimonialSectionService';
...
export default function TestimonialSectionManager() {
  ...
  const fetchSection = async () => {
    try {
      setIsLoading(true);
      const res = await getTestimonialSections();
  ...
  const saveSection = async () => {
    ...
    try {
      if (sectionId) {
        await updateTestimonialSection(sectionId, formData);
        toast.success('Section updated successfully!');
      } else {
        await createTestimonialSection(formData);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
      ...
      >
        <div>
          <h3 className="text-lg font-bold text-gray-800">Testimonials Section</h3>
