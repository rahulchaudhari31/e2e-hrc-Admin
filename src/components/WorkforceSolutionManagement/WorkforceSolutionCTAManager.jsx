...
} from '../../services/workforceSolution/workforceSolutionCTAService';
...
export default function WorkforceSolutionCTAManager() {
  ...
  const fetchCTA = async () => {
    try {
      setIsLoading(true);
      const res = await getAdminWorkforceSolutionCTA();
  ...
  const saveCTA = async () => {
    ...
    try {
      ...
      if (ctaId) {
        await updateWorkforceSolutionCTA(ctaId, payload);
        toast.success('CTA updated successfully!');
      } else {
        const response = await createWorkforceSolutionCTA(payload);
